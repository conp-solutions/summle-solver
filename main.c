...
// if we do not use CBMC, read data from stdin
    ...
    for (T i = 0; i < maxsteps; ++i) {
        if (C_n_ops[i] >= 4) {
            printf("operation[%d]=%d exceeds minimum (4)\n", i, C_n_ops[i]);
            exit(0);
        }
    }
    ...
    for (T i = 0; i < maxsteps && !invalid; ++i) {
        ...
        if (invalid) {
            printf("Failed A_pos[%d]=%d minimum (%d) check\n", i, A_n_pos[i], m + i);
            exit(0);
        }
        invalid = invalid || !!(A_n_pos[i] >= position_pool_size); // TODO: check off-by-one!
        if (invalid) {
            printf("Failed A_pos[%d]=%d maximum check\n", i, A_n_pos[i]);
            exit(0);
        }
        ...
        if (invalid) {
            printf("Failed A-cross-B duplicate check A_pos[%d]=B_pos[%d], (%d == %d) maximum check\n", i, i, A_n_pos[i],
                   B_n_pos[i]);
            exit(0);
        }
        ...
        for (T j = i + 1; j < maxsteps && !invalid; ++j) {
            ...
            if (invalid) {
                printf("Failed A duplicate check A_pos[%d]=A_pos[%d], (%d == %d) maximum check\n", i, j, A_n_pos[i], A_n_pos[j]);
                exit(0);
            }
    ...
    for (T i = 0; i < maxsteps && !invalid; ++i) {
        ...
        if (invalid) {
            printf("Failed B_pos[%d]=%d minimum check\n", i, B_n_pos[i]);
            exit(0);
        }
        invalid = invalid || !!(B_n_pos[i] >= position_pool_size); // TODO: check off-by-one!
        if (invalid) {
            printf("Failed B_pos[%d]=%d maximum check\n", i, B_n_pos[i]);
            exit(0);
        }
        ...
        for (T j = i + 1; j < maxsteps && !invalid; ++j) {
            ...
            if (invalid) {
                printf("Failed B duplicate check B_pos[%d]=B_pos[%d], (%d == %d) maximum check\n", i, j, B_n_pos[i], B_n_pos[j]);
                exit(0);
            }
            ...
            if (invalid) {
                printf("Failed B-cross-A duplicate check A_pos[%d]=B_pos[%d], (%d == %d) maximum check\n", i, j,
                       A_n_pos[i], B_n_pos[j]);
                exit(0);
            }
        }
    ...
    T found_solution = 0;
    for (T i = 0; i < maxsteps; ++i) {
        if (A_n_pos[i] != 0 && B_n_pos[i] != 0) found_solution = found_solution || (C_n[i] == X);
    }
    ...
    if (found_solution) {
        ...
        for (T i = 0; i < maxsteps; ++i) {
            if (A_n_pos[i] != 0 && B_n_pos[i] != 0) {
                printf(" %d    %s    %d   =    %d\n", values[A_n_pos[i]], ops[C_n_ops[i]], values[B_n_pos[i]], C_n[i]);
            }
        }
        printf("X = %d\n", X);
